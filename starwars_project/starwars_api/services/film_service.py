...
from starwars_api.models import Film, Vote
...
def list_film_and_vote(request):

    films = []
    try:
        films = Film.objects.all().order_by('episode_id')
    except Exception as e:
        logger.error(StarWarsLogMessages.DATABASE_ERROR_FILM + str(e))
        messages.error(request, StarWarsMessages.DATABASE_ERROR_FILM)
